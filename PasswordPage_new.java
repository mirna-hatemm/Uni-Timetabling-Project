...
public class PasswordPage_new extends Composite {
	...
	public PasswordPage_new() {
		...
		if ("1".equals(Window.Location.getParameter("reset"))) {
			...
			// Password Recovery using Mobile Number
			iPhone = new TextBox();
			iPhone.setStyleName("gwt-SuggestBox");
			iForm.addRow(MESSAGES.fieldPhoneNumber(), iPhone);

			iFooter.addButton("reset", MESSAGES.buttonReset(), new ClickHandler() {
				@Override
				public void onClick(ClickEvent event) {
					iFooter.clearMessage();
					if (iPhone.getValue().isEmpty()) {
						iFooter.setErrorMessage(MESSAGES.errorPhoneNumberNotValid());
					} else {
						iFooter.setEnabled("reset", false);
						RPC.execute(PasswordChangeRequest.resetPassword(iPhone.getValue()),
								new AsyncCallback<PasswordChangeResponse>() {
									@Override
									public void onFailure(Throwable caught) {
										iFooter.setEnabled("reset", true);
			...
			iFooter.addButton("change", MESSAGES.buttonChange(), new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					if (iOldPassword.getValue().isEmpty()) {
						...
						RPC.execute(PasswordChangeRequest.changePassword(
										iUser == null ? null : iUser.getValue(),
										iOldPassword.getValue(),
										iNewPassword.getValue(),
										""),
								new AsyncCallback<PasswordChangeResponse>() {
									@Override
									public void onFailure(Throwable caught) {
			...
			if (Window.Location.getParameter("user") != null) {
				iUser = new TextBox();
				iUser.setStyleName("gwt-SuggestBox");
				iUser.setValue(Window.Location.getParameter("user"));
				iForm.addRow(MESSAGES.fieldUsername(), iUser);
			}

			iOldPassword = new PasswordTextBox();
			iOldPassword.setStyleName("gwt-SuggestBox");
			...
			iNewPassword = new PasswordTextBox();
			iNewPassword.setStyleName("gwt-SuggestBox");
			iForm.addRow(MESSAGES.fieldNewPassword(), iNewPassword);

			iRetypePassword = new PasswordTextBox();
			iRetypePassword.setStyleName("gwt-SuggestBox");
	...
	public static class PasswordChangeRequest implements GwtRpcRequest<PasswordChangeResponse> {
		private String iUsername, iEmail, iOldPassword, iNewPassword, iPhone;
		...
		public boolean hasPhone() {
			return iPhone != null && !iPhone.isEmpty();
		}
		...
		public boolean hasUsername() {
			return iUsername != null && !iUsername.isEmpty();
		}
		...
		public boolean hasEmail() {
			return iEmail != null && !iEmail.isEmpty();
		}
		...
		public static PasswordChangeRequest changePassword(String username, String oldPasswd, String newPasswd, String phone) {
		...
		public static PasswordChangeRequest resetPassword(String phone) {
			PasswordChangeRequest req = new PasswordChangeRequest();
			req.setPhone(phone);
			req.setReset(true);
			return req;
